...
export function Page({ title, kicker, intro, children }: { title: string; kicker: string; intro: string; children: React.ReactNode }) { return <><div className="page-heading"><p className="eyebrow">{kicker}</p><h1>{title}</h1><p className="lede">{intro}</p></div>{children}<footer><span>ClauseRoot v0.1</span><span>State over spectacle.</span><span>GenLayer localnet / chain {LOCALNET_CHAIN_ID_DECIMAL}</span></footer></>; }
